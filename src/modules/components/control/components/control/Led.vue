                    ...
                    <div class="row" style="margin: 20px;"> 
                        <div class="col-xs-6">
                            ...
                            <label class="toggle">
                             ...
                             <label class="toggle" v-show="!auto">
...
export default {
  ...
  methods: {
    addTimer: function() {
      ...
      let t = [stmin, spmin];
      this.control.timer.list.push(t);
    },
    deleteTimer: function(index) {
      this.control.timer.list.splice(index, 1);
    },
    update: function() {
      this.control = JSON.parse(JSON.stringify(this.GET_CONTROL[0]));
      this.timerList = this.control.timer.list;

      this.activation = this.control.mode?1:0;
      this.auto = this.control.mode?1:0;
      this.status = this.control.manual.status?1:0
    },
    submit: function() {
      this.GET_CONTROL[0].timer.list = this.control.timer.list;
      this.GET_CONTROL[0].mode = this.auto?1:0;
      this.GET_CONTROL[0].manual.status = this.status?1:0;
  ...
  watch: {
    ...
    status: function(data) {
      var status = data ? 1 : 0;
      this.control.manual.status = status;
    },
    auto: function(data) {
      var mode = data ? 1 : 0;
      this.control.mode = mode;
