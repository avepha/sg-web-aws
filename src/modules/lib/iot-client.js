...
export default class IoTClient {
  ...
  initClient(options) {
    ...
    const clientShadowId = `chat-user-${Math.floor((Math.random() * 1000000) + 1)}`;

    this.shadow = DeviceSdk.thingShadow({
  ...
  /**
   * Register thing shadow
   * 
   * @param {string} thingName -thingName
   */
  registerThing(thingName, onRegistered) {
    this.shadow.register(thingName, {}, onRegistered)
  }
  /**
   * Attach a delta handler
   * 
   * @param {string} thingName -thingName
   */
  attachThingDelta(onDeltaAttached) {
    this.shadow.on('delta', onDeltaAttached);
  }
  /**
   * Attach a status handler
   * 
   * @param {string} thingName -thingName
   */
  attachThingStatus(onStatusAttached) {
    this.shadow.on('status', onStatusAttached);
  }

  attachThingConnect(onConnectAttached) {
    this.shadow.on('connect', onConnectAttached);
  }

  attachThingError(onErrorAttached) {
    this.shadow.on('error', onErrorAttached);
  }

  requestShadow(thingName) {
    this.shadow.get(thingName)
  }



  updateThing(thingName, data) {
    var state = { state: { desired: data } }
    console.log(state);
    var clientTokenId = this.shadow.update(thingName, state)
